...
# hold out in train/ or outside train --> contain_e/no_e
...
output_file='new_data/'+args.input_file.split('/')[-1].split('.')[0]+rp+'_'+can_name+'_tokenized_all.json' #  'dev_contain_e_valid_cands_tokenized_all.json'/'dev_contain_e_candis_tokenized.json'
with open(output_file,'w') as f:
    json.dump(new_list,f)

output_file='new_data/'+args.input_file.split('/')[-1].split('.')[0]+rp+'_'+can_name+'_tokenized_mul.json' #  'dev_contain_e_valid_cands_tokenized_mul.json'/'dev_contain_e_candis_tokenized_mul.json'
with open(output_file,'w') as f:                                                                  #  'dev_contain_e_rep_valid_cands_tokenized_mul.json' (token replaced)
    json.dump(new_list_multi,f)

output_file='new_data/'+args.input_file.split('/')[-1].split('.')[0]+rp+'_'+can_name+'_tokenized_sing.json' #  'dev_contain_e_valid_cands_tokenized_mul.json'/'dev_contain_e_candis_tokenized_mul.json'
with open(output_file,'w') as f:                                                                  #  'dev_contain_e_rep_valid_cands_tokenized_mul.json' (token replaced)
    json.dump(new_list_multi,f)


if contain:
    output_file='new_data/{}{}_contain_e.json'.format(mode,rp)  # dev_rep_contain_e.json/dev_contain_e.json
else:
    output_file='new_data/{}{}_no_e.json'.format(mode,rp)
with open(output_file,'w') as f:
