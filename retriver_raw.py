...
parse.add_argument('--mode', type=str,default='dev' ,help='dev/test/no_e')
parse.add_argument('--input_test_file', type=str, help='already contain triples')
parse.add_argument('--topK_sens',  type=int, default=50,help='retrive topK sens each sample')
...
pre='/data/disk3/private/xuweijia/my_drqa_up/data/final_test/'
if args.mode=='dev':
    args.input_test_file=pre+'dev_contain_e.json' # dev_contain_e_valid_cands_tokenized_all.json
elif args.mode=='test':
    args.input_test_file=pre+'test_contain_e.json'
elif args.mode=='no_e':
...
new_folder='data/final_test_{}/'.format(args.topK_sens)
import subprocess
...
#    else:
    ...
    if len_s >0:
        ...
        print('mode:{},n:{},n_s:{},time={:.3f}'.format(args.mode,n,n_s,(end-start)/3600))
        sample_new=dict()
...
print({'mode:':args.mode},{'topn:':args.topK_sens})
# dev_contain_e_ndoc50_raw.json
# no_e_ndoc50_raw.json
output_file=new_folder+'{}_contain_e_ndoc{}_raw.json'.format(args.mode,args.topK_sens) if args.mode!='no_e' else new_folder+'no_e_ndoc{}_raw.json'.format(args.topK_sens)
